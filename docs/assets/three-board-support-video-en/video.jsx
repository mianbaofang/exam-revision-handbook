...
function IntroScene() {
  return <Scene start={0} end={5.6}><div className="kicker">Open-source revision handbook Skill</div><h1 className="headline infographic-headline">Turn official course requirements into handbooks students can really learn from.</h1><p className="lead">GCSE, IGCSE, A-Level, and AP with explicit boundaries for sources, writing, visuals, review, and PDF delivery.</p><div className="imagegen-visual provider-art"><img src="../intro-providers-infographic.png" alt="" /><div className="visual-legend provider-legend">{boards.map((board, index) => <Stagger start={0.9} index={index} key={board.name}><span style={{ "--accent": board.accent }}>{board.name}</span></Stagger>)}</div></div></Scene>;
}
