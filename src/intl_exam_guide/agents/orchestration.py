...
OPTIONAL_MULTI_AGENT_MODE = "optional-user-requested"
    ...
    return [
        ...
        AgentRole(
            ...
            dispatch_brief=[
                ...
                "Write source-bound PedagogicalUnit content, practice, mastery summaries, and per-topic visual_decision records.",
                "Do not approve the final output; hand the rendered handbook to the Reviewer for visible inspection.",
            ],
        ),
        AgentRole(
...
def agent_orchestration_payload(
    ...
    return {
        ...
        "mode": OPTIONAL_MULTI_AGENT_MODE,
        "multi_agent_required": False,
        "agent_runtime_contract": {
            "automatic_dispatch": (
                "Separate-agent dispatch is optional and user-requested. The default Skill "
                "workflow is the lightweight Analyst -> Writer -> Reviewer sequence."
            ),
            "fallback_without_subagents": (
                "Use the same host LLM sequentially while keeping Analyst, Writer, and Reviewer "
