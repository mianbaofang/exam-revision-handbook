...
def build_concept_writing_prompt(
    ...
    return "\n".join(
        [
            ...
            "",
            "6. JUDGE VISUAL NEEDS AFTER WRITING EACH TOPIC",
            "   - First finish the source-bound explanation, worked example, solution, pitfall, and mastery_summary.",
            "   - Then ask whether a visual materially improves this completed topic.",
            "   - If NO: do not output visual_spec.",
            "   - If YES: first try an LLM-authored exact SVG only when labels and geometry fully carry the teaching meaning.",
            '   - Use complexity: "svg-basic" only for exact-fit diagrams, with svg_fit: "exact" and review_status guidance for LLM review.',
            "   - If LLM review rejects the SVG, route the same teaching need to Kroki for a professional diagram and review that output.",
            '   - If Kroki review also fails or the visual needs realism, rich annotation, apparatus, scenes, or modelling nuance, use complexity: "infographic" so it goes to a reviewed image asset route.',
            ...
            "",
            "3. Do NOT output visual_spec before writing the topic content.",
            ...
            "- Python may render the handbook after Analyst and Writer artifacts exist, using the configured board-level-subject-time HTML/PDF names",
            ...
            "- Quality Inspector checks package completeness only; Reviewer must open the rendered HTML/PDF and audit the visible handbook",
            "",
            "MEMORY & CONSISTENCY:",
            f"- Writing style: {style} ({style_desc})",
